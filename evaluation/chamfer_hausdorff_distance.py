...
def reconstruct_tools_from_depth_mask(depth_path, mask_path, camera_intrinsics):
    ...
    depths = depth_image[ys, xs].astype(np.float32) # unit mm
...
def find_numbered_matches(depth_dir: str, mask_dir: str, rgb_dir: str, pcd_dir: str) -> List[Tuple[str, str, str, str]]:
    depth_map = _collect_numbered(depth_dir, (".png", ".exr", ".tiff"))
    mask_map  = _collect_numbered(mask_dir, (".png", ".jpg"))
    rgb_map   = _collect_numbered(rgb_dir, (".png", ".jpg"))
    pcd_map   = _collect_numbered(pcd_dir, (".ply", ".pcd", ".xyz"))
    common = sorted(set(depth_map) & set(mask_map) & set(rgb_map) & set(pcd_map))
    return [(depth_map[i], mask_map[i], rgb_map[i], pcd_map[i]) for i in common]

def process_dataset(pcd_dir, depth_dir, mask_dir, rgb_dir, intrinsics_json):
    fx, fy, cx, cy = load_camera_intrinsics(intrinsics_json)
    pairs = find_numbered_matches(depth_dir, mask_dir, rgb_dir, pcd_dir)
    chamfers = []
    hausdorffs = []
    saved_sample = False
    for depth_path, mask_path, rgb_path, ref_path in pairs:
        # Load rgb just to verify; ignore if missing
        _ = cv2.imread(rgb_path, cv2.IMREAD_COLOR)
        recon_pts = reconstruct_tools_from_depth_mask(depth_path, mask_path, (fx, fy, cx, cy))
...
def main():
    pcd_dir = "/workspace/EndoLRMGS/stereomis/p1/initial_tools"
    depth_dir = "/workspace/datasets/endolrm_dataset/stereomis/p1/depth"
    mask_dir = "/workspace/datasets/endolrm_dataset/stereomis/p1/binary_mask_deva"
    rgb_dir = "/workspace/datasets/endolrm_dataset/stereomis/p1/left_finalpass"
