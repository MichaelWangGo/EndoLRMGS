...
class SCARED_Dataset(object):
    ...
    def load_meta(self):
        ...
        calibs_dir = osp.join(self.root_dir, "data", "frame_data")
        rgbs_dir = osp.join(self.root_dir, "data", "left_finalpass")
        depths_dir = osp.join(self.root_dir, "data", "depth")  # Changed from disparity to depth
        masks_dir = osp.join(self.root_dir, "data", "binary_mask_deva")
        frame_ids = sorted([id[:-5] for id in os.listdir(calibs_dir)])
        # frame_ids = frame_ids[::self.skip_every]
        n_frames = len(frame_ids)
        ...
        for i_frame in trange(n_frames, desc="Process frames"):
            ...
            depth_path = osp.join(depths_dir, f"{frame_id}.png")
            depth = iio.imread(depth_path).astype(np.float32)
            depth = depth / 1000.0  # Convert to meters if depth is in millimeters
            depth[depth > self.depth_far_thresh] = 0
            depth[depth < self.depth_near_thresh] = 0
            depths.append(depth)
            
            # Load mask
            mask_path = osp.join(masks_dir, f"{frame_id}.png")
            if os.path.exists(mask_path):
                mask = np.array(Image.open(mask_path))/255.0
                kernel = np.ones((47, 47), np.uint8)
                mask = cv2.dilate(mask, kernel, iterations=1)
                mask = 1 - mask  # Invert mask to match other datasets
            else:
                # If no mask exists, create one from depth
                mask = (depth != 0).astype(float)
                kernel = np.ones((int(depth.shape[1]/128), int(depth.shape[1]/128)), np.uint8)
...
class Stereomis_Dataset(object):
    ...
    def load_meta(self):
        ...
        if os.path.exists(os.path.join(self.root_dir, "binary_mask_deva")):
            ...
            self.masks_paths = None  # No masks available
        ...
        # Reference pose for normalization (first frame)
        first_frame_name = os.path.basename(self.image_paths[0])
        if first_frame_name in pose_data:
            c2w0 = pose_data[first_frame_name]
        else:
            # Fallback to identity if first frame not found
            c2w0 = np.eye(4)
            print(f"Warning: First frame {first_frame_name} not found in poses file")
        
        for idx in range(n_frames):
            frame_name = os.path.basename(self.image_paths[idx])
            
            if frame_name in pose_data:
                c2w = pose_data[frame_name]
                # Normalize relative to first frame
                c2w = np.linalg.inv(c2w0) @ c2w
            else:
                print(f"Warning: Frame {frame_name} not found in poses file, using identity")
                c2w = np.eye(4)
    ...
    def get_init_pts(self, sampling='random'):
        ...
        for idx in self.train_idxs:
            ...
            num_pts = pts.shape[0]
            if sampling == 'fps':
                sel_idxs = fpsample.bucket_fps_kdline_sampling(pts, int(0.05*num_pts), h=3)
            elif sampling == 'random':
                sel_idxs = np.random.choice(num_pts, int(0.005*num_pts), replace=False)
            else:
                raise ValueError(f'{sampling} sampling has not been implemented yet.')
            pts_sel, colors_sel = pts[sel_idxs], colors[sel_idxs]
            pts_total.append(pts_sel)
