...
from FMGaussianSplatting.scene.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, \
    read_extrinsics_binary, read_intrinsics_binary, read_points3D_binary, read_points3D_text
from FMGaussianSplatting.utils.graphics_utils import getWorld2View2, focal2fov
import numpy as np
from plyfile import PlyData, PlyElement
from FMGaussianSplatting.scene.gaussian_model import BasicPointCloud
from FMGaussianSplatting.utils.general_utils import PILtoTorch
...
def readEndoNeRFInfo(datadir, mode):
    # load camera infos
    from FMGaussianSplatting.scene.endo_loader import EndoNeRF_Dataset
...
def readScaredInfo(datadir, mode, init_pts, with_mask=False):
    # load camera infos
    from FMGaussianSplatting.scene.endo_loader import SCARED_Dataset
...
def readStereomisFInfo(datadir):
    # load camera infos
    from FMGaussianSplatting.scene.endo_loader import Stereomis_Dataset
    endo_dataset = Stereomis_Dataset(
        datadir=datadir,
        downsample=1.0
    )
    train_cam_infos = endo_dataset.format_infos(split="train")
