...
class BaseDataset(torch.utils.data.Dataset, ABC):
    ...
    @staticmethod
    def _load_rgb_image(file_path, bg_color: float = 1.0):
        ''' Load and blend RGB image with a certain background, 0-1 scaled '''
        # 加载并归一化图像
        rgb = np.array(Image.open(smart_open(file_path, 'rb')))
        rgb = torch.from_numpy(rgb).float() / 255.0  # 转为 0-1 范围的浮点数
        rgb = rgb.permute(2, 0, 1).unsqueeze(0)  # 调整维度：从 (H, W, 3) 转为 (1, 3, H, W)
        return rgb
